...
We performed the following procedures in cleaning or data:
...
```{r calculate-state-pop-per-race}
...
As we can see, white offenders committed more crimes than any other race in every year, almost tripling every other race combined.
...
top_5_bias <- top_5_bias %>%
    arrange(desc(x)) %>%
    slice(1:5) 
head(top_5_bias$Bias)
```

This shows the proportion of the type of Bias description from 2010-2019. We see that around 1/3 of hate crimes committed are towards African-American people. Also, we see from the table that the other top biases are Anti-Gay, Anti-White, Anti-Hispanic, and Anti-LGBT.
...
Before we created our models, we had to perform a couple of minor data pre-processing actions since which included the following:
...
We split the data 70% training and 30% testing. We performed this split twice, one with the data set not containing `STATE_NAME`, `POP_STATE` or `POP_NAME` and the other containing all information as the RandomForest model had a maximum level cap of 32. 
...
# Hate2 contains both pop_state and pop_name
# Remove STATE_ABBR because we already have STATE_NAME
...
```{r rf-importance-per-race}
importance(rf.hate)
```

For our random forest model, we set `mtry=3` since the model took 30min to process! We used data from the first split as there are too many factor levels for all state variables. Cross-validation is not needed to evaluate the accuracy of our model since we can use the Out-of-bag error to assess the accuracy of our model. We see that the Random Forest model has performed successfully well with having only \~23% error rate! We see that `BIAS_DESC`, `LOCATION_NAME`, and `DATA_YEAR` are the top 3 importance factors for our random forest. Furthermore, we see from the plotted random forest model that there is an extremely small error accuracy for predicting white offenders and a somewhat reasonable error accuracy for predicting black offenders, but a high error rate for other racial groups.
...
For our boosted model, we used the `gbm()` as it is able to compute multinomial distributions efficiently. We used a total of 500 trees for the model. We performed cross-validation by creating a for loop that saves the model and its accuracy for each fold. Since `gbm()` is able to also compute factor variables with a high amount of levels, we ran the model on both splits of data. With 5 folds, the Boosted model took 30 min to cross validate for both splits and had an error rate of 23.7%. `BIAS_DESC`, `STATE_NAME`, `OFFENSE_NAME`, and `LOCATION_NAME` were key predictors in the creation of our model.
...
Our logistic regression performed just as well as our other models, regardless of including `STATE_NAME` or not.
...
Our Naive-Bayes Classifier ran the fastest out of every model. Considering it had a similar error rate to the other models, it is the most efficient.

# Conclusion

We think that the Random Forest model and gradient boosting algorithm performed extremely well since they are great with high dimensional data since they only work with subsets of data. It is faster to train than normal decision trees because we are working only on a subset of features in this model, so we can easily work with hundreds of features at a time. That being said, the performance time of Naive-Bayes was the fastest model to build, with an error rate not much worse than our other models. Another thing worth mentioning is that white offenders made up around 705 of our data, so most of our models could simply guess white for any crime and already have a fairly high accuracy rate. Our models technically aren't wrong, but it raises the question of whether or not it is viable to try and predict an offender's race when the data is so skewed toward white offenders.

We learned that a majority of our machine learning models use BIAS_DESC and OFFENSE_NAME as key predictors when classifying offender race. Furthermore, our model had a high percentage of observations where white offenders committed a majority of Anti-Black crimes and less observations for offenders who are people of color (and Anti-Black was the most common bias description). Thus, our models can have more bias due to the distribution of offender's race for Anti-Black hate crimes. In addition, we are not working with all hate crimes that have occured in the US as the data we trained our model on is only from FBI recorded hate crimes. 
