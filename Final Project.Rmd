...
```{r}
california <- pop_full %>% filter(NAME == 'California') %>% select_if(is.numeric)
number <- colSums(california)
number
```
...
hc_1 <- select(hc_full, select = -c(INCIDENT_ID, ORI, PUB_AGENCY_NAME, PUB_AGENCY_UNIT,
                                    DIVISION_NAME, AGENCY_TYPE_NAME, OFFENDER_ETHNICITY))
...
bias = as.factor(hc_2$BIAS_DESC)
hc_2$bias <- bias

dat <- model.matrix(bias~ OFFENSE_NAME + LOCATION_NAME + VICTIM_TYPES + OFFENDER_RACE + STATE_NAME + POPULATION_GROUP_CODE, hc_2) 
train = sample(nrow(dat), 4000)

x.train = dat[train, ]
y.train = hc_2[train, ]$bias

# The rest as test data
x.test = dat[-train, ]
y.test = hc_2[-train, ]$bias
```

```{r}
mlr <- multinom(bias ~ OFFENSE_NAME + LOCATION_NAME + VICTIM_TYPES + OFFENDER_RACE + STATE_NAME + POPULATION_GROUP_CODE, hc_2)
summary(mlr)
str(hc_2)
```

Possibility: Use Neural Netowrks or Multinomial Logistic Regression for modeling biases.
